...
assert.equal(parsed.includes[0].quoted, "%L")
assert.equal(parser.parseIncludeDirective('include "%L"'), "%L")
assert.equal(parser.parseIncludeDirective("include %L"), null)
assert.equal(parser.expandIncludeTemplate("%H/.XCompose", { H: "/home/user" }), "/home/user/.XCompose")
assert.equal(parser.expandIncludeTemplate("%S/%L", { S: "/usr/share/X11/locale", L: "en_US.UTF-8/Compose" }), "/usr/share/X11/locale/en_US.UTF-8/Compose")
assert.equal(parser.expandIncludeTemplate("%%H", { H: "/home/user" }), "%H")
assert.equal(parser.lookupComposeDir("en_US.UTF-8/Compose\ten_US.UTF-8\nC/Compose\tC\n", "en_US.UTF-8"), "en_US.UTF-8/Compose")
assert.ok(parser.localeAliases("en_US.utf8").indexOf("en_US.UTF-8") >= 0)

const combined = parser.combine([
  parser.parse('<Multi_key> <a> <a> : "å"', "/tmp/system.XCompose"),
  parser.parse('<Multi_key> <a> <a> : "custom"\n<Multi_key> <z> <z> : "ž"', "/tmp/user.XCompose")
])
assert.equal(combined.entries.length, 2)
assert.equal(combined.entries.find(function(entry) { return entry.compactSequence === "aa" }).result, "custom")
assert.ok(combined.diagnostics.some(item => item.code === "overridden-sequence" && item.relatedSource === "/tmp/system.XCompose"))

const unquoted = parser.parse("include %L\n<Multi_key> <x> : \"x\"")
assert.ok(unquoted.diagnostics.some(item => item.code === "invalid-include"))
assert.equal(unquoted.entries.length, 1)
