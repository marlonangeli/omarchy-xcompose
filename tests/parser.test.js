...
assert.equal(parsed.entries[0].id, "x216f7502dc9c4646")
...
const longName = "n".repeat(parser.maxMetadataName + 1)
const longTag = "t".repeat(parser.maxMetadataItem + 1)
const cappedMetadata = parser.parse(`# @name: ${longName} @tag: ${longTag}\n<Multi_key> <n> : "n"`).entries[0]
assert.equal(cappedMetadata.name.length, parser.maxMetadataName)
assert.equal(cappedMetadata.tags[0].length, parser.maxMetadataItem)
const unicodeName = "n".repeat(parser.maxMetadataName - 1) + "😀x"
const cappedUnicodeName = parser.parse(`# @name: ${unicodeName}\n<Multi_key> <u> : "u"`).entries[0].name
assert.equal(Array.from(cappedUnicodeName).length, parser.maxMetadataName)
assert.equal(cappedUnicodeName.endsWith("😀"), true)
assert.equal(parser.compactPreview("😀".repeat(81), 80), "😀".repeat(79) + "…")
assert.equal(parser.compactPreview("a \n b", 80), "a ↵ b")
assert.equal(parser.compactPreview("a \t b", 80), "a ⇥ b")
assert.equal(parser.compactPreview("a \r\n b", 80), "a ↵ b")
